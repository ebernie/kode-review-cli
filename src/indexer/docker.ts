...
/**
 * Build the list of candidate paths to check for bundled Docker assets.
 *
 * Exported for test access. Every entry must be derived from `__dirname`
 * (the installed module location). `process.cwd()`-based fallbacks are
 * deliberately omitted — see `getDockerAssetsPath` for the threat model.
 */
export function getDockerAssetsCandidates(): string[] {
  return [
 ...
 *
 * Searches paths relative to the installed module (`__dirname`) only.
 * Resolving from `process.cwd()` is rejected on purpose: when a user runs
 * `kode-review` inside an attacker-controlled repository whose layout
 * happens to include `src/indexer/docker/compose.yaml`, that file would
 * otherwise be handed to `docker compose -f` and executed. The installed
 * package is the only trusted source for these assets.
 */
function getDockerAssetsPath(): string {
  const possiblePaths = getDockerAssetsCandidates()
 ...
 * this constant so the two can never drift (which is exactly how stale
 * generated files used to leak through "complete" cleanups).
 *
 * Deliberately excluded from this list (present in `src/indexer/docker/`
 * but not copied):
 *   - `test_*.py` — Python unit tests for the indexer; needed only at
 *     build time inside the container, not at runtime in the user's
 *     config dir.
 *   - `__pycache__` — compiled Python bytecode cache.
 *
 * Exported for tests; not intended to be mutated at runtime.
 */
export const BUNDLED_DOCKER_ASSETS: readonly string[] = [
  'compose.yaml',
  'Dockerfile',
  'main.py',
  'indexer.py',
...
function ensureDockerAssets(): string {
  ...
  for (const file of BUNDLED_DOCKER_ASSETS) {
...
export async function cleanupIndexer(): Promise<void> {
  ...
  // Derive the removal list from BUNDLED_DOCKER_ASSETS so the copy/remove
  // halves can't drift. `.env` is added on top because it is generated by
  // writeEnvFile (not copied in from the bundle) but must still be cleaned.
  logger.info('Removing indexer configuration files...')
  const filesToRemove = [...BUNDLED_DOCKER_ASSETS, '.env']
